...
with open('cleaned-wiki.json') as fopen:
    wiki = json.load(fopen)
len(wiki)

with open('wiki-dictionary.json') as fopen:
    dic = json.load(fopen)

dictionary = dic['dictionary']
rev_dictionary = dic['rev_dictionary']
...
bi = BidirectionalLMDataset(wiki, uni)
...
saver.save(sess, 'elmo-256/model.ckpt')
...
def _get_feed_dict_from_X(X, model, char_inputs, bidirectional):
    ...
    if bidirectional:
        if not char_inputs:
            ...
            feed_dict[model.tokens_characters_reverse] = X[
...
for p in pbar:
    ...
    feed_dict.update(
        _get_feed_dict_from_X(batch, model, char_inputs, bidirectional)
    )
...
freeze_graph('elmo-256', strings)

import pickle

with open('elmo-256.p', 'wb') as fopen:
    pickle.dump(
        {
            'dictionary': dictionary,
            'rev_dictionary': rev_dictionary,
            'softmax_weight': model.softmax_W.eval(session = sess),
        },
        fopen,
    )
