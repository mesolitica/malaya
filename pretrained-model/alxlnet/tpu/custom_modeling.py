    ...
    with tf.variable_scope(scope, reuse = reuse):
        ...
        if use_tpu:
            ...
            if one_hot_idx.shape.ndims == 2:
                o = tf.einsum('in,nd->id', one_hot_idx, lookup_table)
                return (
                    tf.matmul(o, project_variable),
                    lookup_table,
                    project_variable,
                )
            else:
                o = tf.einsum('ibn,nd->ibd', one_hot_idx, lookup_table)
                o = tf.einsum('ibn,nd->ibd', o, project_variable)
                return (o, lookup_table, project_variable)
        else:
