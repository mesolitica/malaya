...
def get_channel_embeddings(
    io_depth, targets, hidden_size, name = 'channel', vocab_size = 256
):
    """Get separate embedding for each of the channels."""
    targets_split = tf.split(targets, io_depth, axis = 3)
    rgb_embedding_var = tf.get_variable(
        'rgb_target_emb_%s' % name, [vocab_size * io_depth, hidden_size]
    )
...
def right_shift_blockwise_nd(x, block_shape):
  ...
  """
    ...
    blocked_x = tf.reshape(
        blocked_x, [blocked_x_shape[0], -1, blocked_x_shape[-1]]
    )
...
def multihead_attention_nd_partial(hparams):
    ...
    multihead_fn = functools.partial(
        ...
        sparsity_cluster_attention_window = hparams.sparsity_cluster_attention_window,
        sparsity_cluster_strided_num_heads = hparams.sparsity_cluster_strided_num_heads,
        sparsity_cluster_strided_relative = hparams.sparsity_cluster_strided_relative,
...
def transformer_decoder_layers(
    inputs,
    num_layers,
    hparams,
    losses,
    ...
    with tf.variable_scope(name, default_name = default_name):
        ...
        for c in sequence:
            if c == 'a':
                ...
                x = layer_norm(
                    x, depth, epsilon, layer_collection = layer_collection
                )
            elif c == 'd':
...
def layer_preprocess(layer_input, hparams, layer_collection = None):
  ...
  """
    assert (
        'a' not in hparams.layer_preprocess_sequence
    ), 'No residual connections allowed in hparams.layer_preprocess_sequence'
    assert (
        'z' not in hparams.layer_preprocess_sequence
    ), 'No residual connections allowed in hparams.layer_preprocess_sequence'
...
def multihead_attention_nd(
    query_antecedent,  # pylint: disable=dangerous-default-value
    memory_antecedent,
    total_key_depth,
    total_value_depth,
    output_depth,
    ...
    with tf.variable_scope(
        ...
        if decode_step is not None:
            latest_antecedent = get_item_at_decode_step(
                query_antecedent, decode_step, query_shape
            )
            latest_q, latest_k, latest_v = compute_qkv(
                latest_antecedent,
                memory_antecedent,
                total_key_depth,
                total_value_depth,
            ...
            if memory_antecedent is None:
                k = put_item_in_decode_step(
                    k, latest_k, decode_step, query_shape
                )
                v = put_item_in_decode_step(
                    v, latest_v, decode_step, query_shape
                )
            cache['q'] = q
            cache['k'] = k
            cache['v'] = v
...
def decode_step_to_index(decode_step, query_shape, tensor_shape):
  ...
  """
    ...
    final_index = [
        w + b * q
        for w, b, q in zip(within_block_index, block_index, query_shape)
    ]
...
def put_item_in_decode_step(x, item, decode_step, query_shape):
  ...
  """
    ...
    updated_x = inplace_ops.alias_inplace_update(
        flattened_x, flattened_index, item
    )
...
def split_heads_nd(x, num_heads):
  ...
  """
    ...
    return tf.transpose(
        split_last_dimension(x, num_heads),
        [0, num_dimensions + 1]
...
def combine_heads_nd(x):
  ...
  """
    ...
    return combine_last_two_dimensions(
        tf.transpose(
            x,
            [0] + list(range(2, num_dimensions + 2)) + [1, num_dimensions + 2],
        )
    )
...
def break_into_blocks_nd(x, block_shape):
  ...
  """
    ...
    return tf.reshape(
        ...
        + [np.prod(block_shape, dtype = np.int32)]
        + x_shape[-1:],
    )
...
def break_into_memory_blocks_nd(x, query_shape, memory_flange, masked = False):
  ...
  """
    ...
    blocks_in_memory_flange = [
        m // b for b, m in zip(query_shape, memory_flange)
    ]
    num_query_blocks = [
...
def select_block_for_decode_step(blocked_x, decode_step, query_shape):
  ...
  """
    ...
    return tf.slice(
        blocked_x,
        [0] + blocked_index + [0, 0],
        [blocked_x_shape[0]] + [1] * len(blocked_index) + blocked_x_shape[-2:],
    )
...
def unflatten_blocks_nd(x, blocks_per_dimension):
  ...
  """
    x_shape = shape_list(x)
    print(x_shape, blocks_per_dimension)
...
def causal_attention_bias_nd(
    query_shape,  # pylint: disable=dangerous-default-value
    memory_flange,
    decode_step = None,
    bias_cache = {},
):
  ...
  """
    ...
    prev_blocks = (
        np.prod([2 * b + 1 for b in blocks_per_memory_flange], dtype = np.int32)
        // 2
    )
    all_blocks = np.prod(
    ...
    if decode_step is None:
        center_block_attn = attention_bias_lower_triangle(
            items_in_block, bias_cache
        )
    else:
        step_in_block = decode_step % items_in_block
        cond = tf.reshape(
            tf.less_equal(
                tf.range(items_in_block, dtype = tf.int32), step_in_block
            ),
            [1, 1, items_in_query, items_in_block],
        )
        center_block_attn = tf.where(
            cond,
            tf.zeros([1, 1, items_in_query, items_in_block]),
...
def attention_nd(
    q,  # pylint: disable=dangerous-default-value
    k,
    v,
    query_shape,
    memory_query_shape,
  ...
  """
    ...
    with tf.variable_scope(
        ...
        if decode_step is not None:
            ...
            latest_q = tf.reshape(
                latest_q, [-1, num_heads] + shape_list(latest_q)[1:]
            )
        ...
        if sparsity_cluster_num_heads:
            ...
            output_cluster, cluster_loss, cluster_attn_weights = clustered_local_attention_helper(
                q = q_cluster,
                k = k_cluster,
                v = v_cluster,
                query_shape = query_shape,
        ...
        if sparsity_cluster_strided_num_heads:
            ...
            q_cluster, q = tf.split(
                q,
                [sparsity_cluster_strided_num_heads, remainder_num_heads],
                axis = 1,
            )
            k_cluster, k = tf.split(
                k,
            ...
            output_cluster, cluster_loss, cluster_strided_attn_weights = clustered_local_attention_helper(
                q = q_cluster,
                k = k_cluster,
                v = v_cluster,
                query_shape = query_shape,
        ...
        if decode_step is not None:
            ...
            if start_head:
                _, q = tf.split(
                    latest_q, [start_head, remainder_num_heads], axis = 1
                )
            else:
                q = latest_q
        ...
        if local_num_heads:
            ...
            if decode_step is not None:
                k = select_block_for_decode_step(
                    k, decode_step, mem_query_shape
                )
                v = select_block_for_decode_step(
                    v, decode_step, mem_query_shape
                )
            ...
            def break_bias_into_blocks(bias):
                ...
                bias = break_into_memory_blocks_nd(
                    bias, mem_query_shape, memory_flange, masked = masked
                )
                if decode_step is not None:
                    bias = select_block_for_decode_step(
                        bias, decode_step, mem_query_shape
                    )
            ...
            if token_bias is not None:
                ...
                token_bias_weight = tf.get_variable(
                    name = 'token_bias_weight',
                    initializer = 1.0,
            ...
            output = tf.reshape(
                output, [q_shape[0], local_num_heads] + shape_list(output)[1:]
            )
        ...
        if decode_step is None:
            ...
            output_shape = shape_list(output)
            output = tf.slice(
                output, [0] * len(output_shape), [-1, -1] + q_shape[2:-1] + [-1]
            )
            if decoding_stats is not None:
                if local_num_heads:
                    decoding_stats[
                        '%s/local_local_jsd' % scope_name
                    ] = tf.constant(0.0)
                if local_num_heads and sparsity_cluster_num_heads:
                    decoding_stats[
                        '%s/local_cluster_jsd' % scope_name
                    ] = tf.constant(0.0)
                if local_num_heads and sparsity_cluster_strided_num_heads:
                    decoding_stats[
                        '%s/local_strided_jsd' % scope_name
                    ] = tf.constant(0.0)
                if sparsity_cluster_num_heads:
                    decoding_stats[
                        '%s/cluster_cluster_jsd' % scope_name
                    ] = tf.constant(0.0)
                if (
                    sparsity_cluster_num_heads
                    and sparsity_cluster_strided_num_heads
                ):
                    decoding_stats[
                        '%s/cluster_strided_jsd' % scope_name
                    ] = tf.constant(0.0)
                if sparsity_cluster_strided_num_heads:
                    decoding_stats[
                        '%s/strided_strided_jsd' % scope_name
                    ] = tf.constant(0.0)

        if decode_step is not None and decoding_stats is not None:
            seq_length = np.prod(k_shape[2:-1])
            if local_num_heads:
                local_attn_weights = tf.reshape(
                    local_attn_weights, [q_shape[0], local_num_heads, -1]
                )
                # scatter the attention weights into [batch, heads, seq_length]
                block_len = shape_list(local_attn_weights)[-1]
                batch_idx = tf.reshape(
                    tf.range(q_shape[0]), [q_shape[0], 1, 1, 1]
                )
                batch_idx = tf.tile(
                    batch_idx, [1, local_num_heads, block_len, 1]
                )
                head_idx = tf.reshape(
                ...
                local_attn_weights = tf.scatter_nd(
                    idx,
                    local_attn_weights,
                    [q_shape[0], local_num_heads, seq_length],
                )
            if sparsity_cluster_num_heads:
            ...
            if local_num_heads and sparsity_cluster_num_heads:
                decoding_stats[
                    ...
                    local_attn_weights[:, 0], cluster_attn_weights[:, 0]
                )
            if local_num_heads and sparsity_cluster_strided_num_heads:
                decoding_stats[
                    '%s/local_strided_jsd' % scope_name
                ] += jensen_shannon_divergence(
                    local_attn_weights[:, 0], cluster_strided_attn_weights[:, 0]
                )
            if sparsity_cluster_num_heads:
                decoding_stats[
                    '%s/cluster_cluster_jsd' % scope_name
                ] += jensen_shannon_divergence(
                    cluster_attn_weights[:, 0], cluster_attn_weights[:, 1]
                )
            if (
                sparsity_cluster_num_heads
                and sparsity_cluster_strided_num_heads
...
def local_attention_bias_nd(
  ...
  """
    cache_key = 'local_attention_bias_{}_{}_{}_{}'.format(
        query_shape, memory_flange, masked, cache_padding_bias
    )
    ...
    if masked:
        ...
        causal_attn_bias, padding_attn_bias = maybe_tile(
            causal_attn_bias, padding_attn_bias
        )
...
def strided_local_attention_helper(
    q,  # pylint: disable=dangerous-default-value
    k,
    v,
    query_shape,
  ...
  """
    ...
    with tf.variable_scope(
        ...
        if decode_step is not None:
            ...
            causal_attn_bias = tf.where(
                cond,
                tf.zeros_like(cond, dtype = tf.float32),
...
def clustered_local_attention_helper(
    q,  # pylint: disable=dangerous-default-value
    k,
    v,
    query_shape,
  ...
  """
    ...
    with tf.variable_scope(
        ...
        if hash_items:
            q_idx = hash_items_fn(
                q, sparsity_cluster_size, shape_list(q)[-1], decode_step, name
            )
            if share_qk:
                k_idx = q_idx
            else:
            ...
            clustering_loss = tf.constant(0.0)
        else:
            ...
            if share_qk:
                k_cluster_dists, k_clustering_loss = (
                    q_cluster_dists,
                    q_clustering_loss,
                )
            else:
        ...
        if relative_attention:
            ...
            v_shape = shape_list(v)
            q = tf.reshape(
                q, [q_shape[0], q_shape[1] * q_shape[2]] + q_shape[3:]
            )
            k = tf.reshape(
                k, [k_shape[0], k_shape[1] * k_shape[2]] + k_shape[3:]
            )
            v = tf.reshape(
                v, [v_shape[0], v_shape[1] * v_shape[2]] + v_shape[3:]
            )
        ...
        if decode_step is not None:
            ...
            output = tf.reshape(
                output,
                [batch, num_heads] + [1] * len(query_shape) + v_shape[-1:],
            )
...
def clustered_attention_bias_nd(
    attention_window,  # pylint: disable=dangerous-default-value
    clustered_v,
    masked = True,
    cache_padding_bias = False,
    bias_cache = {},
...
def _generate_relative_positions_matrix(
    length_q, length_k, max_relative_position, query_shape, decode_step = None
):
    ...
    if decode_step is None:
        ...
        distance_mat = (
            tf.expand_dims(tf.range(-length_k, 0, 1), 0) + positive_positions
        )
...
def dot_product_attention_relative(
  ...
  """
    ...
    with tf.variable_scope(
        ...
        return (
            _relative_attention_inner(weights, v, relations_values, False),
            weights,
        )
...
def scatter_cluster_items(clustered_x, idx, seq_length):
  ...
  """
    ...
    res = tf.scatter_nd(
        ...
        tf.scatter_nd(
            idx,
            tf.ones_like(clustered_x),
            [batch, heads, seq_length, x_shape[-1]],
        )
        + 1e-2
    )
    return res
...
def gather_hashed_attention_items(
    q,
    k,
    v,
    idx,
  ...
  """
    ...
    # [batch, heads, num_clusters, attention_window, 1]
    batch_idx = tf.tile(
        batch_idx, [1, heads, sparsity_cluster_size, attention_window, 1]
    )
    # we also need to add head index to idx.
    head_idx = tf.reshape(tf.range(0, heads), [1, heads, 1, 1, 1])
    head_idx = tf.tile(
        head_idx, [batch, 1, sparsity_cluster_size, attention_window, 1]
    )
...
def gather_cluster_attention_items(
    q,
    k,
    v,
    cluster_dists,
  ...
  """
    ...
    # [batch, heads, num_clusters, attention_window, 1]
    batch_idx = tf.tile(
        batch_idx, [1, heads, num_clusters, attention_window, 1]
    )
    # we also need to add head index to idx.
    head_idx = tf.reshape(tf.range(0, heads), [1, heads, 1, 1, 1])
...
def hash_items_fn(
    items, sparsity_cluster_size, codebook_depth, decode_step, name
):
  ...
  """
    with tf.variable_scope(name, reuse = tf.AUTO_REUSE):
        ...
        means = tf.get_variable(
            ...
            trainable = True,
            dtype = tf.float32,
            initializer = lambda shape, dtype = None, partition_info = None, verify_shape = None: layer_norm(  # pylint: disable=g-long-lambda
                tf.random.normal(
        ...
        if ema:
            ...
            with tf.colocate_with(means):
                ...
                ema_means = tf.get_variable(
                    name = 'ema_means',
                    shape = None
                    if isinstance(means, tf.Variable)
                    else [num_heads, sparsity_cluster_size, codebook_depth],
                    trainable = False,
                    initializer = means.initialized_value()
                    if isinstance(means, tf.Variable)
                    else tf.constant_initializer(0),
                )
  ...
  """
    with tf.variable_scope('clustering', reuse = tf.AUTO_REUSE):
        ...
        if ema and mode == tf.estimator.ModeKeys.TRAIN and not is_recomputing:
            ...
            count = tf.reduce_sum(
                tf.reshape(
                    x_means_hot, shape = [-1, num_heads, sparsity_cluster_size]
                ),
                axis = 0,
            )
            if use_tpu:
            ...
            updated_ema_count = moving_averages.assign_moving_average(
                ema_count, count, decay, zero_debias = False
            )
            # [bs, n, s, k], [bs, n, s, h]
            ...
            updated_ema_count = (
                (updated_ema_count + epsilon)
                / (n + sparsity_cluster_size * epsilon)
                * n
            )
