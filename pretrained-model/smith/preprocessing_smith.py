    ...
    def __str__(self):
        ...
        s += 'masked_lm_labels_1: %s\n' % (
            ' '.join(
                [
                    ...
                    for x in self.masked_lm_labels_1
                ]
            )
        )
        s += 'input_mask_1: %s\n' % (
        ...
        s += 'masked_lm_labels_2: %s\n' % (
            ' '.join(
                [
                    tokenization.printable_text(x)
                    for x in self.masked_lm_labels_2
                ]
            )
        )
        s += 'input_mask_2: %s\n' % (
...
def add_features_for_one_doc(
    ...
    if masked_lm_labels:
        ...
        features['masked_lm_positions_' + doc_index] = utils.create_int_feature(
            masked_lm_positions
        )
        features['masked_lm_ids_' + doc_index] = utils.create_int_feature(
            masked_lm_ids
        )
        features['masked_lm_weights_' + doc_index] = utils.create_float_feature(
            masked_lm_weights
        )
...
def create_instance_from_wiki_doc_pair(
    instance_id,
    doc_match_label,
    doc_one_tokens,
    doc_two_tokens,
...
def get_tokens_segment_ids_masks(
    max_sent_length_by_word,
    max_doc_length_by_sentence,
    doc_one_tokens,
    masked_lm_prob,
    ...
    for block_index in range(len(sent_block_token_list)):
        tokens_block, segment_ids_block, masked_lm_positions_block, masked_lm_labels_block, input_mask_block, masked_lm_weights_block = get_token_masks_paddings(
    ...
    assert (
        len(tokens_doc) == max_sent_length_by_word * max_doc_length_by_sentence
    )
    assert (
        len(masked_lm_labels_doc)
...
def get_token_masks_paddings(
    ...
    if max_predictions_per_seq > 0:
        (
            ...
            masked_lm_positions,
            masked_lm_labels,
        ) = utils.create_masked_lm_predictions(
    ...
    if max_predictions_per_seq > 0:
        ...
        masked_lm_positions = [
            (i + max_sent_length_by_word * block_index)
...
#     max_predictions_per_seq = (
#         FLAGS.max_predictions_per_seq if FLAGS.add_masks_lm else 0
#     )
