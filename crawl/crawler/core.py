...
def extract_links(content):
    ...
    for (link, date) in zip(links_list, dates_list):
        try:
            date = date.split('-')
            if date[1].find('hour') >= 0 or date[1].find('minute') >= 0 or date[1].find('分鐘') >= 0 or date[1].find('小時') >= 0:
                date[1] = today
...
def google_news_run(keyword, limit=10, year_start=2010, year_end=2011, debug=True, sleep_time_every_ten_articles=0):
    ...
    while num_articles_index < limit:
        ...
        try:
            ...
            if str(response.content).find('In the meantime, solving the above CAPTCHA will let you continue to use our services') >= 0:
                print('whops, blocked')
                return results
            links = extract_links(response.content)
            nb_links = len(links)
            if nb_links == 0 and num_articles_index == 0:
                print('No results fetched. Either the keyword is wrong or you have been banned from Google. Retry tomorrow or change of IP Address.')
                return results
            if nb_links == 0:
