...
1. NLLB-200, MOE, 54.5B, https://tinyurl.com/nllb200moe54bmetrics, bjn_Latn-zsm_Latn, 55.8
2. NLLB-200, Dense, 3.3B, 17.58 GB, https://tinyurl.com/nllb200dense3bmetrics, bjn_Latn-zsm_Latn, None
3. NLLB-200, Dense, 1.3B, 5.48 GB, https://tinyurl.com/nllb200dense1bmetrics, bjn_Latn-zsm_Latn, None
"""
...
def available_huggingface():
    ...
    logger.info('tested on FLORES200 BJN-to-MS (bjn_Latn-zsm_Latn) pair `dev` set, https://github.com/facebookresearch/flores/tree/main/flores200')
...
def huggingface(model: str = 'mesolitica/finetune-translation-austronesian-t5-small-standard-bahasa-cased', **kwargs):
    """
    Load HuggingFace model to translate BJN-to-MS.
    ...
    if model not in _huggingface_availability:
        raise ValueError(
            'model not supported, please check supported models from `malaya.translation.bjn_ms.available_huggingface()`.'
        )
    return load_huggingface.load_generator(model=model, initial_text='terjemah Banjar ke Melayu: ', **kwargs)
