...
def huggingface(
    ...
    **kwargs,
    ...
    """
    Load HuggingFace model to answer abstractive question answers.

    Parameters
    ----------
    model: str, optional (default='mesolitica/finetune-qa-t5-small-standard-bahasa-cased')
        Check available models at `malaya.qa.abstractive.available_huggingface()`.
    ...
    Returns
    -------
    result: malaya.torch_model.huggingface.AbstractiveQA
    """

    if model not in _huggingface_availability and force_check:
        raise ValueError(
            'model not supported, please check supported models from `malaya.qa.abstractive.available_huggingface()`.'
        )

    return load_huggingface.load_abstractive_qa(model=model, **kwargs)
