  ...
  """
    ...
    if input_ids.shape.ndims == 2:
        input_ids = tf.expand_dims(
            input_ids, axis = [-1]
        )  # shape of input_ids is:[ batch_size, seq_length, 1]
    ...
    flat_input_ids = tf.reshape(
        input_ids, [-1]
    )  # one rank. shape as (batch_size * sequence_length,)
    if use_one_hot_embeddings:
        one_hot_input_ids = tf.one_hot(
            flat_input_ids, depth = vocab_size
        )  # one_hot_input_ids=[batch_size * sequence_length,vocab_size]
        output = tf.matmul(
            one_hot_input_ids, embedding_table
        )  # output=[batch_size * sequence_length,embedding_size]
    else:
        output = tf.gather(
            embedding_table, flat_input_ids
...
def embedding_lookup_factorized(
    input_ids,  # Factorized embedding parameterization provide by albert
    vocab_size,
    hidden_size,
    embedding_size = 128,
    initializer_range = 0.02,
    ...
    if use_one_hot_embeddings:
        ...
        output_middle = tf.matmul(
            one_hot_input_ids, embedding_table
        )  # output=[batch_size * sequence_length,embedding_size]
    else:
        output_middle = tf.gather(
            embedding_table, flat_input_ids
        )  # [vocab_size, embedding_size]*[batch_size * sequence_length,]--->[batch_size * sequence_length,embedding_size]
    ...
    output = tf.matmul(
        output_middle, project_variable
  ...
  """
    ...
    for layer_idx in range(num_hidden_layers):
        ...
        with tf.variable_scope(
            name_variable_scope,
            reuse = True
            if (share_parameter_across_layers and layer_idx > 0)
            else False,
        ):
...
def prelln_transformer_model(
    ...
    for layer_idx in range(num_hidden_layers):
        ...
        with tf.variable_scope(idx_scope['layer'], reuse = tf.AUTO_REUSE):
            ...
            with tf.variable_scope(
                idx_scope['attention'], reuse = tf.AUTO_REUSE
            ):
                attention_heads = []
            ...
            with tf.variable_scope(idx_scope['output'], reuse = tf.AUTO_REUSE):
                ...
                # The activation is only applied to the "intermediate" hidden layer.
            with tf.variable_scope(
                idx_scope['intermediate'], reuse = tf.AUTO_REUSE
            ):
                intermediate_output = tf.layers.dense(
