...
def get_assignment_map_from_checkpoint(
    tvars, init_checkpoint, num_of_group=0
):
    ...
    for name in name_to_variable:
        if name in init_vars_name:
            ...
            re.sub(r'/group_\d+/', '/group_0/', six.ensure_str(name))
            in init_vars_name
            and num_of_group > 1
        ):
            tvar_name = re.sub(
                r'/group_\d+/', '/group_0/', six.ensure_str(name)
            )
        elif (
            re.sub(r'/ffn_\d+/', '/ffn_1/', six.ensure_str(name))
            in init_vars_name
            and num_of_group > 1
        ):
            tvar_name = re.sub(r'/ffn_\d+/', '/ffn_1/', six.ensure_str(name))
        elif (
            re.sub(r'/attention_\d+/', '/attention_1/', six.ensure_str(name))
            in init_vars_name
            and num_of_group > 1
        ):
            tvar_name = re.sub(
        ...
        if num_of_group > 0:
            ...
            for gid in range(1, num_of_group):
                if (
                    ('/group_' + str(gid) + '/' in name)
                    or ('/ffn_' + str(gid) + '/' in name)
...
def get_timing_signal_1d_given_position(
    channels, position, min_timescale=1.0, max_timescale=1.0e4
):
  ...
  """
    ...
    scaled_time = tf.expand_dims(tf.to_float(position), 2) * tf.expand_dims(
  ...
  """
    ...
    if bias is not None:
        ...
        if len(from_shape) == 4:
            ...
            broadcast_ones = tf.ones(
                [from_shape[0], 1, from_shape[2], from_shape[3], 1], tf.float32
            )

        bias = tf.matmul(
            broadcast_ones, tf.cast(bias, tf.float32), transpose_b=True
        )
