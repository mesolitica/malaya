...
@dataclass
class DataCollatorForSeq2SeqTag:
    """
        ...
        if labels is not None:
            ...
            for feature in features:
                ...
                if isinstance(feature["labels"], list):
                    ...
                    feature["labels_tag"] = (
                        feature["labels_tag"] + remainder if padding_side == "right" else remainder + feature["labels_tag"]
                    )
                elif padding_side == "right":
                    feature["labels"] = np.concatenate([feature["labels"], remainder]).astype(np.int64)
                    feature["labels_tag"] = np.concatenate([feature["labels_tag"], remainder]).astype(np.int64)
                else:
                    feature["labels"] = np.concatenate([remainder, feature["labels"]]).astype(np.int64)
                    feature["labels_tag"] = np.concatenate([remainder, feature["labels_tag"]]).astype(np.int64)
...
def main():
    ...
    model = T5Tagging.from_pretrained(
    ...
    def preprocess_function(examples):
        prefixes = [ex['prefix'] for ex in examples["translation"]]
        prefix = tokenizer.encode(prefixes[0], add_special_tokens=False)
        data = [json.loads(ex['data']) for ex in examples["translation"]]
        ...
        for no, row in enumerate(data):
            ...
            for i in range(len(row[0])):
                t = tokenizer.encode(row[0][i][0], add_special_tokens=False)
                tag.extend([row[1][i][1]] * len(t))
                y.extend(t)
                t = tokenizer.encode(row[1][i][0], add_special_tokens=False)
                x.extend(t)
    ...
    if data_args.pad_to_max_length:
        ...
        data_collator = DataCollatorForSeq2SeqTag(
            tokenizer,
            model=model,
