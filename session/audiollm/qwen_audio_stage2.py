    ...
    def forward(
        ...
        **kwargs,
        ...
        if input_features is not None:
            ...
            if dummy_audio:
                print('dummy audio, using zeros loss')
                dummy_usage = masked_audio_features.sum() * 0.0
            else:
        ...
        if labels is not None:
            ...
            if dummy_audio:
                auto_shift_loss = auto_shift_loss + dummy_usage
...
def main():
    ...
    def collator(batch):
        ...
        if len(input_features):
            ...
            print('no audio in batch, dummy input')
            temp_audio = np.zeros((int(16000 * 0.02),))
            inputs_audio = processor.feature_extractor(
                temp_audio, return_attention_mask=True, 
                sampling_rate=16000,
                padding="max_length", return_tensors = 'pt')
            
            input_ids['input_features'] = inputs_audio['input_features']
            input_ids['feature_attention_mask'] = inputs_audio['attention_mask']
            input_ids['dummy_audio'] = True
        return input_ids
