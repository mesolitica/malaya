...
class EntityNetwork:
    def __init__(
        ...
        self.answer_single = tf.placeholder(
            tf.float32, [None, num_classes], name = 'Placeholder_input_y'
        )
        self.dropout_keep_prob = tf.placeholder(
            tf.float32, name = 'Placeholder_dropout_keep_prob'
        )
        ...
        correct_prediction = tf.equal(
            tf.round(tf.nn.sigmoid(self.logits)), tf.round(self.answer_single)
        )
    ...
    def output_module(self):
        ...
        y = tf.identity(
            tf.matmul(activation, self.R) + self.y_bias, name = 'logits'
        )
        return y
