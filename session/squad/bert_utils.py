...
RawResult = collections.namedtuple(
    'RawResult', ['unique_id', 'start_log_prob', 'end_log_prob']
)
...
def input_fn_builder(
    input_file, seq_length, is_training, drop_remainder, use_tpu, bsz, is_v2
):
...
def write_predictions_v1(
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        if not nbest:
            nbest.append(
                _NbestPrediction(
                    text = 'empty', start_log_prob = 0.0, end_log_prob = 0.0
                )
            )
...
def write_predictions_v2(
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        if (
            null_score_diff_threshold is None
            or score_diff < null_score_diff_threshold
        ):
...
def evaluate_v2(
    ...
    out_eval = make_eva(l_dictexact_thresh, f1_thresh)
