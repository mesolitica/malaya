...
NEEDLE_FORMAT = "\nNombor ajaib Malaysia yang istimewa ialah: {}.\n"
PREFIX = "Ini adalah buku cerita yang sangat panjang: <buku>"
QUESTION_STR = "</buku>.\n Berdasarkan kandungan buku tersebut, Soalan: Apakah nombor ajaib Malaysia? Jawapan: Nombor ajaib Malaysia yang istimewa ialah:"
    ...
    for context_length in tqdm(
        ...
        for depth in np.arange(0, 1 + args.depth_interval, args.depth_interval):
            ...
            for random_number in random_number_list:
                ...
                tokenized_needle = tokenizer.encode(needle_str)
                tokenized_postfix = tokenizer.encode(question_str)
                tokenizer_answer = tokenizer.encode(asnwer_str)
