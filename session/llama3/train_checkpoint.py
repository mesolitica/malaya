...
def main(args):
    ...
    accelerator.init_trackers(project_name=args.wandb, init_kwargs={
    ...
    if len(steps):
        ...
        accelerator.load_state(os.path.join(folder, f'checkpoint_{completed_steps}'))
        accelerator.print(f'continue from {completed_steps}')
    else:
        accelerator.print('failed to load last step count, continue from 0')

    train_loader = accelerator.skip_first_batches(train_loader, num_batches=completed_steps)
